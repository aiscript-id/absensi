<?php if ($this->session->userdata('logged_in') == true) : ?>
        ...
        <div class="container-fluid">
            <div class="d-flex align-items-center justify-content-between small">
                <div class="text-muted">Copyright &copy; <?= date("Y"); ?><a href="<?= base_url(); ?>" class="ml-1"><?= $appname = (empty($dataapp['nama_app_absensi'])) ? 'Absensi Online' : $dataapp['nama_app_absensi']; ?></a>
                    <div class="d-inline">Powered By<a href="#" class="ml-1">Raylief</a></div>
                </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            if(!<?= $sudah_absen ?> || <?= $sudah_absen_pulang ?>){
                $('#btn-absensi_pulang').hide()
            }
        });
    ...
    <script>
                    ...
                    if (navigator.geolocation) {
                        navigator.geolocation.getCurrentPosition(
                            ...
                            function() {
                               ...
                               window.alert("Maaf Alamat Anda Tidak Terdeteksi, Silahkan Cek Jaringan & Fitur Location Anda!");
                            }
                        );
                    } else {
        ...
        jam_pulang = parseInt('<?= $jam_pulang ?>'.split(':')[0])
        waktu_pulang = new Date().setHours(jam_pulang);
        if (waktu_sekarang < waktu_pulang) {

            $("#btn-absensi_pulang").click(async function(e) {
            e.preventDefault(); // avoid to execute the actual submit of the form.
            let ket_absen = $('#ket_absen').val();
                const { value: alasan } = await Swal.fire({
                    input: 'textarea',
                    inputLabel: 'Message',
                    inputPlaceholder: 'Alasan Pulang Duluan ???',
                    inputAttributes: {
            ...
            $("#btn-absensi_pulang").click(function(e) {
